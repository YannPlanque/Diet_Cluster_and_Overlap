##--------------------------------------------------------------------------------------------------------
## SCRIPT : Harbour and grey seal diets from scat content analysis (in the Baie de Somme, France):
##          identifying diet clusters from seal scat contents, and measuring the interspecific overlap
## Specific content : - Diet composition quantified in % by mass of functionnal prey
##                      (identified from hard prey remains, e.g. fish otoliths, cephalopod beaks)
##                    - Measure uncertainty in diet composition (bootstrap procedure, considering the number of samples)                  
##                    - Identification of diet clusters using a hiearchical clustering
##                    - Diet overlap:
##                       • identifying the composition diet cluster clusters
##                       • comparing the distribution of both seal species scats in each cluster 
##                       • calculating Pianka index.
##
## As part of : 
...
##
## Last update : 2021-04-20 (Version 1.3)
##
##
...
### 0 // Useful functions #################################################################################
  ...
  ## n = number of rows in the matrix; m = number of bootstrap iterations
    ...
    ## Input data: a data.frame including the scats, with a column including the associated cluster (cf. "Parameter")
    ## Output: Proportion of samples by parameter with the associated Confidence Interval CI (at Proba%, default 95%)
...
### I // Data ################################################################################################
  ## 1 / Direction
  Direction <- ".../Script_Planque_et_al_Diet_Cluster_Overlap_V1-3"
  #Direction <- "C:/Users/yplanq01/Documents/CEBC/Article_Planque_et_al_Niche_Overlap/Scripts/Diet_Cluster_Overlap/Script_Planque_et_al_Diet_Cluster_Overlap_V1-3"
  ...
  #   Diet matrix in reconstructed mass of functionnal prey (in grams)
  ...
  # Functionnal prey names (data frame)
  ...
  ## 1 / Diet composition  (with a bootstrap procedure)
    ...
    N_scat_year <- as.data.frame(Seal_diet_year_seasons %>% group_by(Seal, Season, Year_grp) %>%
                                      ...
                                      mutate(Seal = factor(Seal, levels=c("Harbour seals", "Grey seals")),
                                             ...
                                             Functionnal_groups = "Small benthic flatfish", Num_group_functionnal=1))
    ...
    Plot_raw_diet_year_seasons <- 
      ...
      ggplot(mapping = aes(x = Year_grp, 
                           y = Prop,
                           fill = reorder(Functionnal_groups, +Num_group_functionnal)))+
...
### III // Identification of diet clusters (hierarchical clustering) #########################################
  ...
  ## Distance: euclidean ; Method = ward.D2
    ...
    ## 3 / Distribution of scats in each diet cluster & the associated composition 
      ...
      All_data_clust$Clust_FFstat <- ifelse(All_data_clust$Cluster %in% c(5,6), 1, 2) # Flatfish clusters, 1 and 2...
      ...
      ggsave(Plot_B_year_season, filename = paste(Direction,"Plot", "Diet_cluster_04_Distrib_year_season_PDF.pdf", sep = "/"), dpi = 600,
             width = 11, height = 6.75)
      ...
      ggsave(Plot_diet_clust, filename = paste(Direction,"Plot", "Diet_cluster_06_Summary_plot_TEST_ccoda_scale.png", sep = "/"), dpi = 600,
             width = 7.5, height = 9.5)
      
      ggsave(Plot_diet_clust, filename = paste(Direction,"Plot", "Diet_cluster_06_Summary_plot_PDF.pdf", sep = "/"), dpi = 600,
             width = 7.5, height = 9.5)
