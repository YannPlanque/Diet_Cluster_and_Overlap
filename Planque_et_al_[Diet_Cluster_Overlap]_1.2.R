...
### I // Data ################################################################################################
  ## 1 / Direction
  Direction <- ".../Script_Planque et al_[Diet_Cluster_Overlap]_V1.2"
  #Direction <- "C:/Users/yplanq01/Documents/CEBC/Article_Planque_et_al [DIET]/Scripts/Diet_Cluster_Overlap/Script_Planque et al_[Diet_Cluster_Overlap]_V1.2"
  ...
  Pv_data$Species <- "Pv"
    ...
    # Create a all-year/season column
    ...
    # C / Prepare matrix for diet analyses
...
### III // Identification of diet clusters (hierarchical clustering) #########################################
  ...
  ## Distance: euclidean ; Method = ward.D2
    ...
    # B / Compute clustering (factoextra and robCompositions packages)
    # K = 6

    All_dtf <- as.data.frame(All_mtx)
    
    # Clustering on compositionnal data with clustCoDa() [in robCompositions package]:
    # // Test with default parameters proposed in clustCoDa
    res.hc <- clustCoDa(x = All_dtf, k = K,
                        scale = "robust", 
                        distMethod = "Aitchison", 
                        transformation = "identity", 
                        method = "Mclust")
      # -> did not converge, likely due to a high number of null values in the raw data
    ...
    # Another classical method to compute clustering:
    # res.hc <- All_mtx %>%
    #   scale() %>%  # Scale data
    ...
    Plot_dendro <- fviz_dend(res.hc$dtree, k=K) # Plot
    Plot_dendro
    ...
    # Clustering validation statistics (silhouette method) (/!\ Only work with classical clustering method)
    ...
    # To visualise mean content of scats by cluster before going further
    All_dtfBIS <- All_dtf
    ...
    ## 3 / Distribution of scats in each cluster & associated scat content
      ...
      # Plot B_Year_season - Proportions of scats in each cluster, by year-season                                      
